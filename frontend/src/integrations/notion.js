...
export const NotionIntegration = ({
  ...
  const handleConnectClick = async () => {
    try {
      ...
      const pollTimer = window.setInterval(() => {
        ...
        setTimeout(handleWindowClosed, 800); // slight delay for Redis sync
      }, 200);
    } catch (e) {
      setIsConnecting(false);
      alert(e?.response?.data?.detail || "Notion connection failed");
    }
  };
  ...
  const handleWindowClosed = async () => {
    try {
        ...
        setIntegrationParams((prev) => ({
          ...
          load: async () => {
            const res = await axios.post(
              "http://localhost:8000/integrations/notion/load",
              { credentials }
            );
            return res.data;
          },
      ...
      setIsConnecting(false);
      alert(
        e?.response?.data?.detail || "Failed to retrieve Notion credentials"
      );
    }
  };
  ...
  useEffect(() => {
    setIsConnected(
      integrationParams?.type === "Notion" && integrationParams?.credentials
        ? true
        : false
    );
  }, [integrationParams]);
